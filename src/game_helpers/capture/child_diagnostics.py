...
def main() -> int:
    ...
    print(f"parent hwnd={parent.hwnd} title={parent.title!r}")
    print(f"WSGAME children discovered: {len(discover_game_views(parent.hwnd))}")

    for view in views:
        print(
            f"\nWSGAME #{view.index}: hwnd={view.hwnd} "
            f"pid={view.window.metadata.get('pid', 'unknown') if hasattr(view.window, 'metadata') else 'unknown'} "
