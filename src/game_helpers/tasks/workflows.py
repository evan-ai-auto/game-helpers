...
def default_workflows():
    return (
        TaskWorkflow(id="minghun_basic_capabilities", name="[未验收] 基础能力列表 - 单项测试最基础可复用能力", description="仅调用现有实现，不修改能力实现本身。业务人工验收状态：未验收。", recipe=TaskRecipe(id="minghun_basic_capabilities", name="[未验收] 基础能力列表 - 单项测试最基础可复用能力", category=TaskCategory.GENERAL, metadata={"game": "梦幻西游", "diagnosis_only": True, "baseline_client": "800x600"})),
