...
def default_workflows():
    return (
        ...
        TaskWorkflow(id="minghun_shortcut_diagnostic", name="命魂快捷图标 Hover / 运动 / 后台输入分层验证", description="单一诊断任务入口；进入后选择完整流程或独立子实验（运动、OCR 分段、Hover、PostMessageW、Click），不修改生产默认配置。", recipe=TaskRecipe(id="minghun_shortcut_diagnostic", name="命魂快捷图标 Hover / 运动 / 后台输入分层验证", category=TaskCategory.GENERAL, metadata={"game": "梦幻西游", "diagnosis_only": True, "calibration": "shortcut_panel_layered_diagnostic", "baseline_client": "800x600", "subtypes": ["full", "motion", "ocr_roi_compare", "hover", "postmessage_hover", "click_hotspot"]})),
