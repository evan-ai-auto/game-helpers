...
def default_workflows():
    return (
        ...
        TaskWorkflow(id="minghun_shortcut_diagnostic", name="命魂快捷图标 Hover / 运动 / 后台输入分层验证", description="单一诊断任务入口；进入后选择完整流程或四个独立子实验，不修改生产默认配置。", recipe=TaskRecipe(id="minghun_shortcut_diagnostic", name="命魂快捷图标 Hover / 运动 / 后台输入分层验证", category=TaskCategory.GENERAL, metadata={"game": "梦幻西游", "diagnosis_only": True, "calibration": "shortcut_panel_layered_diagnostic", "baseline_client": "800x600", "subtypes": ["full", "motion", "hover", "postmessage_hover", "click_hotspot"]})),
