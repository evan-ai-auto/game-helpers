...
class TaskWorkflowRegistry:
    def __init__(self, workflows: tuple[TaskWorkflow, ...] | None = None) -> None:
        self._workflows = workflows or default_workflows(); self._by_id = {w.id:w for w in self._workflows}; self._by_name = {w.name:w for w in self._workflows}
    def all(self): return self._workflows
    def get(self, workflow_id: str): return self._by_id[workflow_id]
    def get_by_name(self, name: str): return self._by_name[name]
