...
A visual state is represented by one or more stable template anchors. Anchors
are searched over the complete captured frame, so the target may move. A
candidate is accepted only when all required anchors occur at their declared
relative offsets from the same origin.
"""
...
def _ncc_map(image: np.ndarray, template: np.ndarray, step: int = 1) -> np.ndarray:
    ...
    shape = (H + h - 1, W + w - 1)
    corr = np.fft.irfftn(
        np.fft.rfftn(image, shape) * np.conj(np.fft.rfftn(template, shape)),
        shape,
    ).real[: H - h + 1, : W - w + 1]
    n = float(h * w)
...
def _load_profile(path: str | Path) -> VisualStateProfile:
    ...
    anchors = tuple(
        VisualAnchor(
            ...
            search_step=int(item.get("search_step", 1)),
        )
        for item in payload["anchors"]
    )
    if not anchors:
        raise ValueError("visual state profile has no anchors")
    return VisualStateProfile(
        name=str(payload["name"]),
        anchors=anchors,
        consecutive_frames=int(payload.get("consecutive_frames", 2)),
    )
...
def detect_visual_state(frame: Frame, profile: VisualStateProfile) -> VisualStateObservation:
    ...
    try:
        ...
        matches: list[tuple[VisualAnchor, np.ndarray]] = []
        for anchor in profile.anchors:
            template = _template_gray(anchor.image_path)
            scores = _ncc_map(image, template, anchor.search_step)
            if scores.size == 0:
                return VisualStateObservation(profile.name, VisualStateStatus.NOT_DETECTED, 0.0, evidence=(f"anchor {anchor.name} cannot fit frame",))
            matches.append((anchor, scores))

        base = profile.anchors[0]
        base_scores = matches[0][1]
        threshold = base.threshold
        ys, xs = np.where(base_scores >= threshold)
        if len(xs) == 0:
            return VisualStateObservation(profile.name, VisualStateStatus.NOT_DETECTED, float(base_scores.max()), evidence=(f"anchor {base.name} not found",))

        candidates = np.argsort(base_scores[ys, xs])[::-1]
        for idx in candidates[:64]:
            bx = int(xs[idx] * base.search_step)
            by = int(ys[idx] * base.search_step)
            scores_for_origin: list[tuple[str, float]] = [(base.name, float(base_scores[ys[idx], xs[idx]]))]
            valid = True
            for anchor, scores in matches[1:]:
                ax = bx + anchor.offset_x - base.offset_x
                ay = by + anchor.offset_y - base.offset_y
                sx = ax // anchor.search_step
                sy = ay // anchor.search_step
                ...
                if score < anchor.threshold:
                    ...
                    break
            if valid:
                confidence = min(score for _, score in scores_for_origin)
                return VisualStateObservation(
                    profile.name,
                    VisualStateStatus.DETECTED,
                    confidence,
                    (bx - base.offset_x, by - base.offset_y),
                    tuple(scores_for_origin),
                    ("all required anchors matched",),
                )

        return VisualStateObservation(
            profile.name,
            VisualStateStatus.NOT_DETECTED,
            float(base_scores.max()),
            evidence=("base anchor found but composite anchors did not align",),
        )
