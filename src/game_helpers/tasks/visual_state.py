...
def _frame_gray(frame: Frame) -> np.ndarray:
    array = np.frombuffer(frame.data, dtype=np.uint8).reshape((frame.height, frame.width, 4))
    # BGRA -> luminance. Ignore alpha because WGC may legitimately expose
    # transparent padding around a hosted child.
...
def _ncc_map(image: np.ndarray, template: np.ndarray, step: int) -> np.ndarray:
    """Normalized cross correlation map using integral window statistics."""
    ih, iw = image.shape
    th, tw = template.shape
    if th > ih or tw > iw:
        return np.empty((0, 0), dtype=np.float32)
    ys = range(0, ih - th + 1, step)
    xs = range(0, iw - tw + 1, step)
    # Direct sliding windows are acceptable for the small UI assets used here;
    # keep the implementation deterministic and independent of OpenCV.
    ...
    if template_norm <= 1e-8:
        return np.zeros((len(range(0, ih - th + 1, step)), len(range(0, iw - tw + 1, step))), dtype=np.float32)
    result = np.empty((len(range(0, ih - th + 1, step)), len(range(0, iw - tw + 1, step))), dtype=np.float32)
    for yi, y in enumerate(ys):
        for xi, x in enumerate(xs):
            window = image[y : y + th, x : x + tw]
            centered = window - float(window.mean())
            denom = float(np.sqrt(np.sum(centered * centered)) * template_norm)
            result[yi, xi] = float(np.sum(centered * template_centered) / denom) if denom > 1e-8 else 0.0
    return result
...
def detect_visual_state(frame: Frame, profile: VisualStateProfile) -> VisualStateObservation:
    ...
    try:
        ...
        base, _base_template, base_scores = matches[0]
            ...
            for anchor, _template, scores in matches[1:]:
                if anchor.position_type == VisualPositionType.FIXED:
                    ...
                    if not anchor_candidates:
                        valid = False
                        break
                    _ax, _ay, score = anchor_candidates[0]
...
def _load_profile(path: str | Path) -> VisualStateProfile:
    ...
    anchors = tuple(
        VisualAnchor(
            name=str(item["name"]),
            image_path=(path.parent / str(item["image"])).resolve(),
...
def make_visual_state_verifier(
    capture: Callable[[], Frame],
    profile: VisualStateProfile,
    *,
    expected_detected: bool = True,
) -> Callable[[], VisualStateObservation | None]:
    """Return a stateful verifier for either a positive or negative visual state."""
    consecutive = 0

    def verify() -> VisualStateObservation | None:
        nonlocal consecutive
        observation = detect_visual_state(capture(), profile)
        matches_expectation = observation.detected == expected_detected
        # Invalid observations never count as evidence for either state.
        if observation.status == VisualStateStatus.INVALID:
            consecutive = 0
        elif matches_expectation:
            consecutive += 1
