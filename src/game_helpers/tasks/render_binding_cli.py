...
def main() -> int:
    ...
    # Do not capture a baseline before switching. WGC is occasionally unable
    # to produce the first frame from an already-hosted window, and the
    # baseline is not identity evidence anyway. Binding verification itself
    # performs the meaningful post-switch capture.
    _log("3/6 后台切换到选中实例", f"WSGAME #{selected.view_index}，同步 Tab 与 Surface")
    _log("4/6 捕获并检查渲染绑定")
    ...
    _log("6/6 完成", "请人工确认 Tab 标题与截图中的角色身份一致")
    time.sleep(0.05)
