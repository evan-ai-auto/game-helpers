"""Legacy verifier for claimed Soul Task icon templates.

This preserves the pre-dcc936c verification strategy:
- each template is searched independently across the full ROI;
- an optional expected location is used as a soft score prior;
- the best template result and cross-template location consistency are reported.
"""
...
def _candidate_score(patch: np.ndarray, template: Template) -> float:
    ...
    edge_score = _masked_edges(patch, template)
    return 0.45 * rgb + 0.30 * structure + 0.25 * edge_score
...
def find_best_match(
    ...
    for y in range(rh - th + 1):
        for x in range(rw - tw + 1):
            candidate = _candidate_score(region[y:y + th, x:x + tw], template)
            absolute = (x0 + x, y0 + y)
            if expected_xy is not None:
                distance = float(np.hypot(absolute[0] - expected_xy[0], absolute[1] - expected_xy[1]))
                candidate -= min(0.12, distance / 500.0)
            if candidate > best_score:
                best_score, best_xy = candidate, absolute
...
def main() -> int:
    ...
    parser.add_argument("--expected", nargs=2, type=int, metavar=("X", "Y"))
    parser.add_argument("--output-dir", type=Path, default=Path("diagnostic/soul_task/template_verify/legacy"))
    ...
    results = [find_best_match(frame, template, tuple(args.roi), expected_xy) for template in templates]
    ...
    if location is not None:
        x, y = location
        tw, th = templates[0].rgb.shape[1], templates[0].rgb.shape[0]
